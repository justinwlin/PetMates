    ...
    <template v-for="(item, index) in list">
      ...
      >
        ...
        <template v-if="index === 0 && status !== 2">
          <span class="pointer-wrap nope-pointer-wrap" :style="{ opacity: nopeOpacity }">
            <slot name="nope" :opacity="nopeOpacity" />
          </span>
          <span class="pointer-wrap like-pointer-wrap" :style="{ opacity: likeOpacity }">
        ...
        <span v-if="state.status === 4" class="pointer-wrap rewind-pointer-wrap">
          <slot name="rewind" />
        </span>
      </TinderCard>
    </template>
